...
class TouchImageView : AppCompatImageView {
    ...
    val scrollPosition: PointF?
        get() {
            ...
            val point = transformCoordTouchToBitmap(
                (viewWidth / 2).toFloat(), (viewHeight / 2).toFloat(), true
            )
        ...
        override fun onScaleEnd(detector: ScaleGestureDetector) {
            ...
            if (animateToZoomBoundary) {
                val doubleTap: DoubleTapZoom = DoubleTapZoom(
                    targetZoom, (viewWidth / 2).toFloat(), (viewHeight / 2).toFloat(), true
                )
                compatPostOnAnimation(doubleTap)
            }
        }
    ...
    private fun scaleImage(
        deltaScale: Double, focusX: Float, focusY: Float, stretchImageToSuper: Boolean
    ) {
    ...
    private inner class DoubleTapZoom internal constructor(
        targetZoom: Float, focusX: Float, focusY: Float, stretchImageToSuper: Boolean
    ) : Runnable {
     ...
     * system of the drawable that the AppCompatImageView contain
    ...
    private inner class ZoomVariables(
        var scale: Float, var focusX: Float, var focusY: Float, var scaleType: ScaleType?
    )
