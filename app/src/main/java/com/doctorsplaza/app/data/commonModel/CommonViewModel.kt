...
class CommonViewModel @Inject constructor(private val repository: Repository,private val session: SessionManager):ViewModel() {
    ...
    fun deleteDoctorAccount() = viewModelScope.launch {
        safeDeleteDoctorCall()
    }


    private suspend fun safeDeleteDoctorCall() {
        deleteDoctorAccount.postValue(Resource.Loading())
        try {
            val response = repository.deleteDoctorAccount(session.loginId.toString())
            if (response.isSuccessful) {
                response.body()?.let { slugsResponse ->
                    deleteDoctorAccount.postValue(Resource.Success(slugsResponse))
                }
            } else {
                deleteDoctorAccount.postValue(Resource.Error(response.message(), null))
            }

        } catch (t: Throwable) {
            when (t) {
                is IOException -> deleteDoctorAccount.postValue(Resource.Error("Network Failure", null))
                else -> deleteDoctorAccount.postValue(
                    Resource.Error(
                        "Conversion Error ${t.message}",
                        null
                    )
