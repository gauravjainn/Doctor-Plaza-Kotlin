...
class BookTimeAdapter @Inject constructor(): RecyclerView.Adapter<BookTimeAdapter.ViewHolder>() {
        ...
        fun bind(data: RoomTimeSlotsData) {
            with(binding){
                ...
                if(data.timeSlotData.isSelected){
                    ...
                    timeAmPm.setTextColor(ContextCompat.getColor(context, R.color.colorPrimaryLight))
                }
   ...
   private val diffUtil = object :DiffUtil.ItemCallback<RoomTimeSlotsData>(){
        override fun areItemsTheSame(oldItem: RoomTimeSlotsData, newItem: RoomTimeSlotsData): Boolean {
            return oldItem._id == newItem._id
        }

        override fun areContentsTheSame(oldItem: RoomTimeSlotsData, newItem: RoomTimeSlotsData): Boolean {
    ...
    private var selectOnTimeClickListener: ((data: RoomTimeSlotsData,position:Int) -> Unit)? = null
