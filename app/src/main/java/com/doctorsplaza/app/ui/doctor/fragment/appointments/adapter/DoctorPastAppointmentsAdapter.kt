...
class DoctorPastAppointmentsAdapter @Inject constructor(): RecyclerView.Adapter<DoctorPastAppointmentsAdapter.ViewHolder>() {
    ...
    inner class  ViewHolder(private val binding: ItemPastAppointmentBinding):RecyclerView.ViewHolder(binding.root){
        fun bind(appointmentData: AppointmentData) {
            with(binding){
                ...
                appointmentData.doctor_id.qualification.forEach {
                ...
                Glide.with(context).load(appointmentData.doctor_id.profile_picture).into(doctorImage)
                consultationFees.text = "₹${appointmentData.consultation_fee}"
                val parsedDate = inputFormat.parse(appointmentData.room_time_slot_id.timeSlotData.createdAt)
                val formattedDate = outputFormat.format(parsedDate)

                appointmentTimings.text ="$formattedDate (${appointmentData.room_time_slot_id.timeSlotData.start_time} - ${appointmentData.room_time_slot_id.timeSlotData.end_time})"


                if(appointmentData.status.lowercase() =="cancelled"){
                    appointmentStatus.text = "Cancel"
                    appointmentStatus.background = ContextCompat.getDrawable(context, R.drawable.red_corner_radius)
                }else if(appointmentData.status.lowercase() =="done"){
                    appointmentStatus.background = ContextCompat.getDrawable(context, R.drawable.gree_corner_radius)
   ...
   private val diffUtil = object :DiffUtil.ItemCallback<AppointmentData>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DoctorPastAppointmentsAdapter.ViewHolder {
        context = parent.context
        return ViewHolder(ItemPastAppointmentBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
